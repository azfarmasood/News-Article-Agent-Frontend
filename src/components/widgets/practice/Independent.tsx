...
export default function IndependentNews() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        setLoading(true)
        const response = await fetch('https://news-agent-production.up.railway.app/independent-news/')
        if (!response.ok) {
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">Independent News</h1>
